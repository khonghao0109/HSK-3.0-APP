...
- Circuit breaker, queue, outbox, shared service or load shedding only when a real failure/scale boundary requires them. Correctness-independent cache may remain local/ephemeral when invalidation and failure behavior are safe; do not add Redis or another service without evidence.
